(() => {
  ...
  let titleTaps = 0;
  let audio, audioUnlocked=false, audioUnlocking=false, musicPlayer, cluePlayer, rewardPlayer, currentMusicKey='welcome';
  const rewardSongUrl='https://audio-ssl.itunes.apple.com/itunes-assets/AudioPreview125/v4/fe/1e/27/fe1e27fa-4872-0b4c-1281-38adf2be1542/mzaf_17246449475060321725.plus.aac.p.m4a';
  ...
  async function playMusic(){
    if(!state.sound||!currentMusicKey)return false;const player=getMusicPlayer(),file=musicFiles[currentMusicKey];const wanted=new URL(`assets/music/${file}.wav?v=soft-music-3`,location.href).href;
  ...
  function stopRewardSong(){if(rewardPlayer)rewardPlayer.pause();}
  async function playRewardSong(){
    if(!state.sound)return false;
    if(!rewardPlayer){rewardPlayer=new Audio(rewardSongUrl);rewardPlayer.loop=true;rewardPlayer.preload='auto';rewardPlayer.volume=.75;rewardPlayer.setAttribute('playsinline','');}
    try{await rewardPlayer.play();return !rewardPlayer.paused;}catch{return false;}
  }
  function startMusic(key){
    stopRewardSong();currentMusicKey=key;if(!state.sound)return;playMusic();
  }
  ...
  document.querySelector('#sound-button').onclick=async()=>{const active=(musicPlayer&&!musicPlayer.paused)||(rewardPlayer&&!rewardPlayer.paused);if(state.sound&&!active){const playing=currentMusicKey?await playMusic():await playRewardSong();document.querySelector('#sound-button').textContent='♪';showToast(playing?'Musica attiva!':'Audio bloccato dal telefono');return;}state.sound=!state.sound;save();document.querySelector('#sound-button').textContent=state.sound?'♪':'×';showToast(state.sound?'Musica e suoni attivi':'Musica e suoni disattivati');if(state.sound){await unlockAudio();tone(520);if(currentMusicKey)startMusic(currentMusicKey);else playRewardSong();}else{stopMusic();stopRewardSong();}};
  document.addEventListener('pointerdown',unlockAudio,{capture:true});
  document.addEventListener('touchend',unlockAudio,{capture:true,passive:true});
  document.addEventListener('keydown',unlockAudio,{capture:true});
  document.addEventListener('visibilitychange',()=>{if(document.hidden){stopMusic();stopRewardSong();}else if(audioUnlocked){if(currentMusicKey)playMusic();else playRewardSong();}});
